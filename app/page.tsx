...
export default function Page() {
  ...
  const parsedCaptionTexts = useMemo(() => {
    if (!apiResult.trim()) return [];
    try {
      const parsed = JSON.parse(apiResult) as unknown;
      ...
      {isAdmin() && (
        <div className="split-layout">
          ...
          <div className="split-column">
            ...
            {parsedCaptionTexts.length > 0 && (
              <div className="grid">
                <h3>Generated captions</h3>
                {parsedCaptionTexts.map((caption, index) => (
                  <div key={`${index}-${caption.slice(0, 16)}`} className="card">
                    {caption}
                  </div>
                ))}
              </div>
